...
<div class="data-management-card" id="records-section" style="margin-top: 1rem;">
    ...
    <div class="table-responsive">
        <table>
            ...
            <tbody>
                <?php if (empty($assets)): ?>
                    <tr><td colspan="9"><div class="empty-state"><p>No claimed records found matching search or filter parameters.</p></div></td></tr>
                <?php else: ?>
    ...
    <?php if ($totalPages > 1): ?>
        <div class="pagination-row" style="margin-top: 1.5rem;">
            ...
            <div class="pagination-buttons">
                <!-- Prev button -->
                <a href="claimed.php?owner_name=<?= urlencode($ownerNameFilter) ?>&id_no=<?= urlencode($idNoFilter) ?>&account_no=<?= urlencode($accountNoFilter) ?>&letter=<?= urlencode($letterFilter) ?>&compilation_start=<?= urlencode($compilationStartFilter) ?>&compilation_end=<?= urlencode($compilationEndFilter) ?>&page=<?= max(1, $page - 1) ?>#records-section" 
                ...
                ?>
                    <a href="claimed.php?owner_name=<?= urlencode($ownerNameFilter) ?>&id_no=<?= urlencode($idNoFilter) ?>&account_no=<?= urlencode($accountNoFilter) ?>&letter=<?= urlencode($letterFilter) ?>&compilation_start=<?= urlencode($compilationStartFilter) ?>&compilation_end=<?= urlencode($compilationEndFilter) ?>&page=<?= $i ?>#records-section" 
                ...
                <!-- Next button -->
                <a href="claimed.php?owner_name=<?= urlencode($ownerNameFilter) ?>&id_no=<?= urlencode($idNoFilter) ?>&account_no=<?= urlencode($accountNoFilter) ?>&letter=<?= urlencode($letterFilter) ?>&compilation_start=<?= urlencode($compilationStartFilter) ?>&compilation_end=<?= urlencode($compilationEndFilter) ?>&page=<?= min($totalPages, $page + 1) ?>#records-section" 
